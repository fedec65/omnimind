 ...
 */
import { MemoryLayer, AgingThresholds, TimeConstants, ok, err, } from '../core/types.js';
/**
 ...
 */
const DEFAULT_COMPRESSION_RULES = [
    // Logical connectors
...
export class AgingPipeline {
    rules;
    constructor(rules = DEFAULT_COMPRESSION_RULES) {
    ...
    shouldAge(memory) {
        if (memory.pinned)
            return false;
        if (memory.layer === MemoryLayer.Wisdom)
            return false;
     ...
     */
    getTargetLayer(memory) {
        const age = Date.now() - memory.createdAt;
        if (age >= AgingThresholds[MemoryLayer.Concept])
            return MemoryLayer.Wisdom;
        if (age >= AgingThresholds[MemoryLayer.Compressed])
            return MemoryLayer.Concept;
        if (age >= AgingThresholds[MemoryLayer.Verbatim])
            return MemoryLayer.Compressed;
     ...
     */
    async transition(memory, targetLayer) {
        try {
            switch (targetLayer) {
    ...
    compressToL1(memory) {
        ...
        if (compressed.length > memory.content.length * 0.5) {
            compressed = memory.content.substring(0, 500) + ' [...]';
        }
        const updated = {
            ...memory,
            content: compressed,
    ...
    inferEntityType(name, context) {
        ...
        if (lower.includes(`class ${nameLower}`) || lower.includes(`interface ${nameLower}`))
            return 'class';
        if (lower.includes(`function ${nameLower}`) || lower.includes(`def ${nameLower}`))
            return 'function';
        if (lower.includes(`import ${nameLower}`) || lower.includes(`from ${nameLower}`))
            return 'module';
        if (lower.includes(`api ${nameLower}`) || lower.includes(`${nameLower} endpoint`))
            return 'api';
        if (lower.includes(`database ${nameLower}`) || lower.includes(`db ${nameLower}`))
            return 'database';
        if (lower.includes(`service ${nameLower}`) || lower.includes(`${nameLower} service`))
            return 'service';
        if (/^[A-Z][a-z]+ [A-Z][a-z]+$/.test(name))
            return 'person'; // "John Smith" pattern
        return 'concept';
    }
...
export class AgingScheduler {
    ...
    start(checkIntervalMs = TimeConstants.DAY) {
        this.stop();
...
//# sourceMappingURL=AgingPipeline.js.map